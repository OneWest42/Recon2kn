...
nodef.write("  <key>order</key><integer>6</integer>\n")
...
pagef = open(ipaddr + "/FTPEnum" + port + "/page.html", "w")
...
pagef.write("<title>NSE: ftp-anon</title>\n")
pagef.write("</head><body><tt><span style=\"font-family: Sans\"><b>nmap -v -p ")
pagef.write(port)
pagef.write(" --script=ftp-anon.nse ")
pagef.write(ipaddr)
pagef.write("</b><br/>\n")
pagef.write("</span></tt><span style=\"font-family: Sans\">\n")

#Run nmap, print out and write to page.html
print "--------------Nmap(ftp-anon) started for %s--------------\n" % (ipaddr)
ftpenum = "nmap -v -p " + port + " --script=ftp-anon.nse " + ipaddr

output = subprocess.check_output(ftpenum, shell=True)
lines = output.split("\n")
for line in lines:
 	print line
	line = line.replace(" ", " &nbsp;")
...
print "--------------Nmap(ftp-anon) finished for %s-------------\n" % (ipaddr)
